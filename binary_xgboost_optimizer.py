...
- Libraries: pandas, scikit-learn, xgboost, optuna, sklearnex (optional, for Intel CPU acceleration).
- A classification dataset in CSV format with a 'target' column.

Script Arguments:
- --dataset: Path to the CSV dataset file.
- --target_list: Comma-separated string specifying the two target classes.
...
Navigate to the directory containing the script and run it with the necessary parameters. Example:
    python binary_xgboost_optimizer.py --fn_data data.csv --target_list "class1,class2" --n_trials 300 --timeout 600

Output:
Saves the the selected features to CSV and the parameters to JSON.
"""
...
def filter_classes(df: pd.DataFrame, target_list: list[str]) -> tuple[pd.DataFrame, list[str]]:
    ...
    if len(classes_to_keep) != 2:
        raise ValueError(f"Please provide exactly two classes to keep from: {og_classes}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fn_data", help="File path for the features dataset", type=str, default="data/features_mean_with_target.csv")
    parser.add_argument("--target_list", help='e.g., --target_list "boys,girls"', type=str, required=True)
